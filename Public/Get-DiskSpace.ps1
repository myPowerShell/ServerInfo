...
﻿<#
.SYNOPSIS
 Get DiskSpace by target % free less than 20% on a Drive, you can set your own threshold
        ...
        $Computer = $Computer.trim()
         ...
         Write-Log ("Currently Processing Server: $Count "+"of "+ $max + "  " + $Computer) -Severity INFO
            ...
            Try{   
                    ...
                    $Items =  Get-CimInstance -CimSession $session cim_logicaldisk -ErrorAction stop |select SystemName, DriveType, VolumeName, Name, @{n='Size_Gb' ; e={"{0:n2}" -f ($_.size/1gb)}}, @{n='FreeSpace_Gb' ; e={"{0:n2}" -f ($_.freespace/1gb)}}, @{n='PercentFree' ; e={"{0:n2}" -f ($_.freespace/$_.size*100)}} | Where-Object {$_.DriveType -eq 3 } #-and [decimal]$_.PercentFree -lt [decimal]$PercentFree}
