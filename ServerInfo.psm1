                    ...
                    @(foreach ($Item in $EventList) {
                            if ($Item.ID -eq 1074) {
                                $Properties = [Ordered]@{
                                    ...
                                    UserName     = $Item.Properties.Value[6]
                                    ShutdownType = $Item.Properties.Value[4]
                                }
                                $Objoutput = New-Object -TypeName PSObject -Property $Properties
                                $Result += $Objoutput
  
                            }
                            elseif ($Item.ID -eq 6008) {
                                $Properties = [Ordered]@{
                                    ComputerName = $Computer
                                    EventID      = $Item.ID
                                    TimeStamp    = $Item.TimeCreated
    ...
    PROCESS {
        ...
        foreach ($computer in $ComputerName) {
            try {
                ...
                Write-Verbose "Entered Disconnected Hosts Section"
                $timestmp = Get-Date -Format "yyyy-MM-dd-hhmmss"
                "$timestmp Get-OSUpdateSummary@ServerInfo RemoteConnectionFailed $Computer" | out-File $ErrorLogFilePath -Append
                $Properties = [Ordered] @{ ComputerName = $Computer
